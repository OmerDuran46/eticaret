...
class StockAlarmPage extends ConsumerStatefulWidget {
  const StockAlarmPage({
    Key? key,
  }) : super(key: key);

  @override
  ConsumerState createState() => _StockAlarmPageState();
}

class _StockAlarmPageState extends ConsumerState<StockAlarmPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: priceAlartWidget(ref.watch(iGetStockAlarmInfoViewModel),ref.watch(iGetApplicationLoginInfoViewModel)),
    );
  }

  Widget priceAlartWidget(IGetStockAlarmInfoViewModel iGetStockAlarmInfoViewModel,IGetApplicationLoginViewModel iGetApplicationLoginViewModel) {
    if (iGetStockAlarmInfoViewModel.getStockAlarmResponse.status == Status.loading) {
      return const Center(
        child: CupertinoActivityIndicator(),
      );
    } else if (iGetStockAlarmInfoViewModel.getStockAlarmResponse.status == Status.completed) {
      var priceAlarmList = iGetStockAlarmInfoViewModel.getStockAlarmResponse.data;
  ...
  getStockAlarm(){
    ref.read(iGetStockAlarmInfoViewModel).getStockAlarm();
  }
  getStockAlarmRefresh(){
    ref.watch(iGetStockAlarmInfoViewModel).getStockAlarm();
  }
